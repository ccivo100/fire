package com.poicom.basic.freemarker.common.handler;
...
public class DirectiveHandler {
	...
	/** This object stores the set of temporary variables created by the template, the value of settings set by the template, the reference to the data model root, etc.<br>该对象存储着一系列临时变量，模板设置值，数据模型的引用等。*/
	private Environment environment;	
	private Map<String, TemplateModel> parameters;
	private TemplateModel[] loopVars;
	/** Represents the nested content of a directive.<br>代表标签的嵌套内容*/
	...
	public void render() throws IOException, TemplateException {
		...
		if (null != templateDirectiveBody)
			templateDirectiveBody.render(environment.getOut());//body.render(env.getOut());
		reduce(reduceMap);
	}
	...
	public String getString(String name) throws TemplateException{
		...
		if(model instanceof TemplateScalarModel){
			...
			return ((TemplateNumberModel)model).toString();
		}else{
			return null;
		}
	}
